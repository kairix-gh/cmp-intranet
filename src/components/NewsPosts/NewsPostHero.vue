<template>
    <router-link :to="{}" class="flex flex-col md:flex-row w-full rounded shadow-lg group md:max-h-[21.25rem] hover:shadow-xl">
        <div class="md:w-1/2">
            <img :src="NewsPost.ImageUrl.Description" class=""/>
        </div>

        <div class="flex flex-col items-start bg-white py-4 px-4 md:w-1/2 relative">
            <span class="text-sm text-gray-400 font-medium uppercase">{{ NewsPost.Category }}</span>
            <p class="text-lg md:text-2xl ">{{ NewsPost.Title }}</p>
            <p class="leading-tight text-xs uppercase mb-1">{{ NewsPost.PublishDate.substring(0, 10) }}</p>
            <p class="text-base mb-12 max-h-32 md:max-h-48 overflow-y-hidden fadeBox">{{ NewsPost.Description }}{{ NewsPost.Description }}</p>
